...
impl State {
    fn render_impl(&mut self, rows: usize, cols: usize) -> String {
        if rows == 0 || cols == 0 {
            return "".to_string();
        }
...
impl ZellijPlugin for State {
    ...
    fn render(&mut self, rows: usize, cols: usize) {
        print!("{}", self.render_impl(rows, cols));
    }
}
...
mod test {
    ...
    fn disable_insta_wasm() {
        // Will fail to find workspace root.
        std::env::set_var("INSTA_WORKSPACE_ROOT", ".");
        // Will fail to write snapshots.
    ...
    fn test_something() {
        ...
        state
            .keybinds
            .push((InputMode::Normal, vec![(Key::Down, vec![Action::Quit])]));
        insta::assert_snapshot!(state.render_impl(3, 30), @r#"
        [33mNormal        [90m│[33mShared       [90m│[0m
          [97mDown ━ [97mQuit [90m│             │[0m
        "#);
    }
}
